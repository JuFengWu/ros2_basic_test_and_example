...
#include <inttypes.h>
#include <memory>
#include "action_test/action/test_fibonacci.hpp"
#include "rclcpp/rclcpp.hpp"
// TODO(jacobperron): Remove this once it is included as part of 'rclcpp.hpp'
#include "rclcpp_action/rclcpp_action.hpp"

using TestFibonacci = action_test::action::TestFibonacci;
rclcpp::Node::SharedPtr g_node = nullptr;


void feedback_callback(
  rclcpp_action::ClientGoalHandle<TestFibonacci>::SharedPtr,
  const std::shared_ptr<const TestFibonacci::Feedback> feedback)
{
  RCLCPP_INFO(
    g_node->get_logger(),
    "Next number in partial_sequence received: %" PRId64,
...
int main(int argc, char ** argv)
{
  ...
  auto action_client = rclcpp_action::create_client<TestFibonacci>(g_node, "test_fibonacci");
  ...
  // Populate a goal
  auto goal_msg = TestFibonacci::Goal();
  goal_msg.order = 10;

  RCLCPP_INFO(g_node->get_logger(), "Sending goal");
  // Ask server to achieve some goal and wait until it's accepted
  auto send_goal_options = rclcpp_action::Client<TestFibonacci>::SendGoalOptions();
  ...
  rclcpp_action::ClientGoalHandle<TestFibonacci>::SharedPtr goal_handle = goal_handle_future.get();
  if (!goal_handle) {
  ...
  rclcpp_action::ClientGoalHandle<TestFibonacci>::WrappedResult wrapped_result = result_future.get();
